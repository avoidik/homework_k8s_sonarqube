...
NAME_SPACE="sonarqube"
HELM_CHART="stable/sonarqube"
HELM_VER="3.4.0"
...
# install helm sonarqube chart
helm install \
  "${HELM_NAME}" \
  "${HELM_CHART}" \
  --values values/sonarqube.yaml \
...
kubectl patch configmap tcp-services \
  -n kube-system \
  -p "{\"data\":{\"${HOST_PORT}\":\"${NAME_SPACE}/${HELM_NAME}-${HELM_NAME}:${HOST_PORT}\"}}"
